...
module Symtable = struct
    ...
    class table = object (self)
        ...
        method insert (def: Ast2.def) =
            try match self#lookup def.id with
            | None   -> let scope = List.hd scopes in scope#insert def
            | Some x -> raise @@ ErrDuplicate (def, x)
            with Failure _ -> raise (ErrCompiler "symtable.insert")
    end
end
...
let rec analyse ast1 = st#down; let ast2 = List.map sem_top ast1 in st#up; ast2
